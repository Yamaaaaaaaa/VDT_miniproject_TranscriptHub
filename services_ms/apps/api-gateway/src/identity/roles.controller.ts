...
export class RolesController {
      ...
      .pipe(
        catchError((err) =>
          throwError(
            () =>
              new HttpException(
                err?.message ?? `Role with ID ${id} not found`,
  ...
  @ApiResponse({
    status: 400,
    description: 'Validation failed or role already exists.',
  })
  create(@Body() createRoleDto: CreateRoleDto) {
    return this.identityService
      .createRole(createRoleDto.name)
      .pipe(
        catchError((err) =>
          throwError(
            () =>
  ...
  update(
    ...
    return this.identityService
      .updateRole(id, updateRoleDto.name)
      .pipe(
        catchError((err) =>
          throwError(
            () =>
  ...
  updatePermissions(
    ...
    return this.identityService
      .updateRolePermissions(id, updateRolePermissionsDto.permissions)
      .pipe(
        catchError((err) =>
          throwError(
            () =>
