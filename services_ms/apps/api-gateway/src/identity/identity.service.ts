...
export class IdentityService {
    ...
    // --- Roles API proxy ---
    findAllRoles() { return this.identityClient.send('find_all_roles', {}); }
    findOneRole(id: number) { return this.identityClient.send('find_one_role', id); }
    createRole(name: string) { return this.identityClient.send('create_role', { name }); }
    updateRole(id: number, name: string) { return this.identityClient.send('update_role', { id, name }); }
    deleteRole(id: number) { return this.identityClient.send('delete_role', id); }
    updateRolePermissions(roleId: number, permissions: string[]) {
        return this.identityClient.send('update_role_permissions', { roleId, permissions });
    }
    ...
    findAllPermissions() { return this.identityClient.send('find_all_permissions', {}); }
    findOnePermission(id: number) { return this.identityClient.send('find_one_permission', id); }
}
