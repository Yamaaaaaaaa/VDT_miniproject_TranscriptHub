...
export class FileController {
  ...
  async uploadSingleFile(
    ...
    const audioFile = await this.fileService.uploadSingleFile(
      file,
  ...
  async initializeUpload(
    ...
    if (!uploaderId)
      throw new BadRequestException('X-User-Id header is missing');
    const initResponse = await this.fileService.initializeUpload(
      dto,
      parseInt(uploaderId, 10),
    );
    return initResponse;
  ...
  async completeUpload(
    ...
    if (!uploaderId)
      throw new BadRequestException('X-User-Id header is missing');
    const audioFile = await this.fileService.completeUpload(
      fileId,
      parseInt(uploaderId, 10),
    );
    return this.mapToResponse(audioFile);
  ...
  async updateMetadata(
    ...
    if (!uploaderId)
      throw new BadRequestException('X-User-Id header is missing');
    const audioFile = await this.fileService.updateMetadata(
      fileId,
      dto.fileName,
      parseInt(uploaderId, 10),
    );
