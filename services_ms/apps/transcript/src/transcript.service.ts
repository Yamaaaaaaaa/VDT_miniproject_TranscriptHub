...
export class TranscriptService {
    ...
    async getTranscriptByAudioFileId(audioFileId: string) {
        ...
        if (!transcript) {
            throw new NotFoundException(`Transcript not found for audio file ${audioFileId}`);
        }
        return transcript;
    }
    ...
    async generateTranscriptAsync(fileId: string) {
        ...
        if (!transcript) {
            ...
            console.log(`Transcript for fileId ${fileId} already exists in state: ${transcript.status}`);
        }
    }

    async generateTranscriptManually(fileId: string) {
        console.log(`Manually requesting transcript generation for fileId: ${fileId}`);

        // Verify file exists in file service
        try {
            const exists = await lastValueFrom(
                this.fileClient.send('check_file_exists', fileId)
            );
            if (!exists) {
    ...
    private triggerBackgroundTranscription(fileId: string, transcriptId: number) {
        setImmediate(async () => {
            console.log(`Background thread started transcription for fileId: ${fileId}, transcriptId: ${transcriptId}`);
            try {
                ...
                console.error(`Failed to transcribe fileId: ${fileId}, transcriptId: ${transcriptId}`, err);
                await this.updateTranscriptStatus(transcriptId, 'FAILED', '', { segments: [] });
            }
        });
    }

    private async updateTranscriptStatus(transcriptId: number, status: string, rawText: string, structuredContent: any) {
        try {
            ...
            console.log(`Updated transcript ID: ${transcriptId} to status: ${status}`);
        } catch (err) {
            console.error(`Error updating transcript ID: ${transcriptId} status in DB`, err);
        }
    }

    private async transcribe(fileId: string): Promise<{ rawText: string; segments: any[] }> {
        ...
        try {
            ...
            console.warn(`Could not retrieve file metadata from file-service for fileId: ${fileId}. Using fallback mimeType.`, error);
        }
        ...
        if (!downloadResponse.ok) {
            throw new Error(`Failed to download file from file-service. HTTP status: ${downloadResponse.status}`);
        }

        const arrayBuffer = await downloadResponse.arrayBuffer();
        const fileBytes = Buffer.from(arrayBuffer);
        const base64Audio = fileBytes.toString('base64');
        console.log(`Successfully encoded audio to Base64 (length: ${base64Audio.length} characters)`);
...
4. CRITICAL: Make sure all string values (especially the "text" fields and "rawText") are properly escaped for JSON. Do not include unescaped double quotes; use \\\" instead. Do not include literal newlines inside strings; use \\n instead.`;
        ...
        if (!response.ok) {
            ...
            throw new Error(`Gemini API returned non-success code: ${response.status} - ${errBody}`);
        }
        ...
        try {
            ...
            console.log(`Completed AI transcription using Gemini for fileId: ${fileId}. Extracted ${result.segments?.length || 0} segments.`);
            return result;
        } catch (parseError) {
