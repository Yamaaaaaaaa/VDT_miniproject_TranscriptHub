...
export class IdentityController {
    ...
    async deleteAccount(@Payload() data: { id: number; requesterId?: number }) {
        return this.identityService.deleteAccount(Number(data.id), data.requesterId ? Number(data.requesterId) : undefined);
    }

    @MessagePattern('update_role_permissions')
    async updateRolePermissions(@Payload() data: { roleId: number; permissions: string[] }) {
        return this.identityService.updateRolePermissions(data.roleId, data.permissions);
    }
