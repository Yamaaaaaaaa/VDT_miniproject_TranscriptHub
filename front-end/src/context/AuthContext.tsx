...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const register = async (username: string, email: string, password: string) => {
    ...
    try {
      ...
      // After successful registration, we can automatically attempt login or let the user login.
      // We will let them redirect to login, but let's log them in immediately if the backend supports password-based login.
      setLoading(false);
    } catch (error) {
      setLoading(false);
      throw error;
