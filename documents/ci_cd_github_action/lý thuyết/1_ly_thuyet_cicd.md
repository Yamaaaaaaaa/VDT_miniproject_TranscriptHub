...
Chào mừng bạn đến với tài liệu hướng dẫn về **CI/CD** sử dụng **GitHub Actions**. Tài liệu này được thiết kế nhằm cung cấp một cái nhìn tổng quan, chuẩn chỉ và hệ thống nhất về khái niệm CI/CD, các giai đoạn trong một đường ống (pipeline) tiêu chuẩn, và định hướng học tập DevOps cho dự án **TranscriptHub**.
...
Một đường ống (Pipeline - đối với GitHub Actions thường gọi là **Workflow**) hoàn chỉnh thường bao gồm 6 giai đoạn cốt lõi sau:
...
* **Hành động**: Workflow lắng nghe sự thay đổi trên Git repository thông qua các sự kiện nội bộ của GitHub (GitHub Events) như `push`, `pull_request`, `workflow_dispatch` (chạy thủ công), v.v.
...
* **Hành động**: Cập nhật ứng dụng trên máy chủ.
  * *Với Docker Compose*: SSH vào VPS sử dụng các Action hỗ trợ SSH (ví dụ: `appleboy/ssh-action`), kéo image mới (`docker compose pull`) và restart service không gây downtime (`docker compose up -d`).
  * *Với Kubernetes*: GitHub Actions sử dụng tệp `kubeconfig` và CLI `kubectl` để cập nhật trực tiếp Image Tag mới vào cụm K8s. K8s sẽ tự động thực hiện **Rolling Update** (khởi động Pod mới chạy ảnh mới, kiểm tra trạng thái Ready, sau đó mới tắt Pod cũ).
...
* **Hành động**: Gửi trạng thái của workflow (Thành công/Thất bại) đến các kênh liên lạc của team (Slack, Telegram, Discord, Email) thông qua các GitHub Actions tương ứng. Đồng thời hệ thống giám sát (Prometheus, Grafana) bắt đầu theo dõi sức khỏe của container mới deploy.
